...
const feeStatusConfig: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  pending: { label: "En attente de paiement", color: "bg-warning text-warning-foreground", icon: <Clock className="w-4 h-4" /> },
  paid: { label: "Frais payés", color: "bg-success text-success-foreground", icon: <CheckCircle2 className="w-4 h-4" /> },
  refused: { label: "Paiement refusé", color: "bg-destructive text-destructive-foreground", icon: <XCircle className="w-4 h-4" /> },
  investigating: { label: "Investigation en cours", color: "bg-info text-info-foreground", icon: <Info className="w-4 h-4" /> },
  expired: { label: "Délai dépassé", color: "bg-destructive text-destructive-foreground", icon: <AlertTriangle className="w-4 h-4" /> },
};
...
const FeePaymentBlock = ({ fees, trackingNumber, onContactSupport }: FeePaymentBlockProps) => {
  ...
  const handlePay = () => {
    ...
    toast({
      ...
      description: `Paiement de ${fees.totalAmount.toLocaleString("fr-FR")} ${fees.currency} effectué avec succès.`,
    });
  };
  ...
  const handleRefuse = () => {
    toast({
      title: "Frais refusés",
      description: "Votre refus a été enregistré. Le colis pourra être retourné ou bloqué.",
      variant: "destructive",
    });
  };
    ...
    <Card className={`p-5 space-y-4 border-2 ${isPending ? "border-warning/50 bg-warning/5" : isPaid ? "border-success/30 bg-success/5" : "border-destructive/30 bg-destructive/5"}`}>
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-display font-semibold text-lg">Frais à payer</h2>
        </div>
      ...
      {fees.deadline && isPending && (
        <div className="flex items-center gap-2 text-sm text-warning">
          ...
          <span>Date limite : {new Date(fees.deadline).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</span>
        </div>
      )}
      ...
      {isPaid && fees.paidAt && (
        <div className="flex items-center gap-2 text-sm text-success">
          ...
          <span>Payé le {new Date(fees.paidAt).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</span>
        </div>
      )}
      ...
      {showConfirm && (
        <div className="rounded-lg border border-accent/30 bg-accent/5 p-4 space-y-3">
          <p className="font-medium text-sm">Confirmer le paiement de {fees.totalAmount.toLocaleString("fr-FR")} {fees.currency} ?</p>
          <p className="text-xs text-muted-foreground">Ce paiement couvrira les frais de douane et permettra la poursuite de la livraison du colis {trackingNumber}.</p>
          <div className="flex gap-3">
            <Button variant="accent" size="sm" onClick={handlePay}>Confirmer et payer</Button>
      ...
      {fees.status === "investigating" && (
        <div className="text-sm text-muted-foreground flex items-center gap-2">
          ...
          Une investigation est en cours concernant ces frais. Vous serez notifié de la résolution.
        </div>
      )}
    </Card>
  );
